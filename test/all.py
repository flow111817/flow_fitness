...
def analyze_frame(frame):
    """分析单帧图像，检测姿态并计数俯卧撑"""
    global pushup_count, is_down_position, start_time, analysis_data, last_save_time
    
    # 如果是第一次调用，记录开始时间
    if start_time == None:
        start_time = time.time()
        last_save_time = start_time  # 初始化最后保存时间
    ...
    # 每隔0.5秒保存一帧（使用单独的时间记录）
    if current_time - last_save_time >= 0.5:
        analysis_data['frames'].append(frame)
...
def realtime_analysis():
    """实时视频分析"""
    global analysis_data, last_save_time
